<div>
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('qr.my_qr_codes') }}</h1>
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row">
        ...
        <select wire:model.live="filterType" class="rounded-lg border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
            <option value="">All Types</option>
            @foreach(\App\Enums\QrCodeType::allTypes() as $qrType)
                <option value="{{ $qrType->value }}">{{ $qrType->label() }}</option>
            @endforeach
        </select>
        <select wire:model.live="filterStatus" class="rounded-lg border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
    ...
    @if($qrCodes->isEmpty())
        <div class="rounded-xl border-2 border-dashed border-gray-300 p-12 text-center">
            ...
            <h3 class="mt-4 text-lg font-medium text-gray-900">{{ __('qr.no_qr_codes') }}</h3>
            <p class="mt-2 text-sm text-gray-500">{{ __('qr.no_qr_codes_desc') }}</p>
                ...
                <div class="group rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition hover:shadow-md">
                    <div class="flex items-start justify-between">
                        <div class="flex-1 min-w-0">
                            <h3 class="truncate text-sm font-semibold text-gray-900">{{ $qr->name }}</h3>
                            <div class="mt-1 flex items-center gap-2">
                                <span class="inline-flex items-center rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-600">
                                    {{ $qr->type->label() }}
                                </span>
                                @if($qr->is_dynamic)
                                    <span class="inline-flex items-center rounded-full bg-primary-100 px-2 py-0.5 text-xs font-medium text-primary-700">Dynamic</span>
                                @endif
                                <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium {{ $qr->status === 'active' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                                    {{ ucfirst($qr->status) }}
                                </span>
                            </div>
                        </div>
                    ...
                    @if($qr->is_dynamic && $qr->shortLink)
                        <div class="mt-3 flex items-center gap-2">
                            <code class="flex-1 truncate rounded bg-gray-50 px-2 py-1 text-xs text-gray-600">{{ $qr->shortLink->getFullUrl() }}</code>
                    ...
                    <div class="mt-4 flex gap-2 border-t pt-4">
                        ...
                        <a href="{{ route('qr-codes.edit', $qr) }}" class="flex-1 inline-flex items-center justify-center gap-1 rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 transition">
                            <i class="fa-solid fa-pen-to-square text-[10px]"></i>
                            {{ __('common.edit') }}
                        </a>
                        @endif
                        <button wire:click="downloadPng({{ $qr->id }})" class="flex-1 inline-flex items-center justify-center gap-1 rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 transition">
                            <i class="fa-solid fa-download text-[10px]"></i>
                            {{ __('common.download') }}
                        </button>
                        @if($qr->is_dynamic)
                        <a href="{{ route('analytics.show', $qr) }}" class="flex-1 inline-flex items-center justify-center gap-1 rounded-lg border border-primary-300 px-3 py-1.5 text-xs font-medium text-primary-700 hover:bg-primary-50 transition">
