<div class="mx-auto max-w-3xl">
    <h1 class="text-2xl font-bold text-gray-900">Bulk Generate QR Codes</h1>
    <p class="mt-2 text-sm text-gray-500">Upload a CSV file with Name and URL columns to generate multiple QR codes at once.</p>
    ...
    <div class="mt-6 rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-900/5">
        @if(!$processing)
            <div class="space-y-4">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mb-2">Format: name, url (one per line, max 500 rows)</p>
                ...
                @if($csvFile)
                    <button wire:click="parseCsv" class="rounded-lg border border-primary-600 px-4 py-2 text-sm font-medium text-primary-600 hover:bg-primary-50 transition">
                        Preview Data
                    </button>
                @endif
                ...
                @if(!empty($parsedItems))
                    <div class="mt-4">
                        <h3 class="text-sm font-semibold text-gray-700">Preview ({{ count($parsedItems) }} items)</h3>
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @foreach(array_slice($parsedItems, 0, 20) as $i => $item)
                                        <tr>
                                            ...
                                            <td class="px-3 py-2 text-gray-600 truncate max-w-xs">{{ $item['url'] }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            @if(count($parsedItems) > 20)
                                <p class="px-3 py-2 text-xs text-gray-400">... and {{ count($parsedItems) - 20 }} more</p>
                            @endif
                        </div>

                        <div class="mt-4 flex items-center justify-between">
                            <p class="text-sm text-gray-500">
                                Credits required: <span class="font-semibold text-primary-600">~{{ count($parsedItems) * 8 }}</span>
                            </p>
        ...
        @else
            <div class="text-center py-8">
                ...
                <p class="mt-4 text-lg font-semibold text-gray-900">Generating QR codes...</p>
                <p class="mt-2 text-sm text-gray-500">This is running in the background. Check your QR codes list shortly.</p>
